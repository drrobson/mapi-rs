...
- [#17](https://github.com/wravery/mapi-rs/pull/17)
- PropTag and PropType should impl Copy
- Bump the outlook-mapi-sys dependency in outlook-mapi
...
- *(deps)* Bump outlook-mapi-sys version to match
- release
...
- Update doc comments listing other default fields that are filled in
- Initialize non-default members in doc tests and assert_eq after as_ptr() cast
- Make fixed offset and flags fields const in sized display table types
- Implement the SizedXXX macros for display table structs
- Doc cleanup pass
- Update code sample in README
- Make SizedXXX macros declare a custom type (like MAPI does)
- Bump outlook-mapi crate version
- Safer PropValue wrapper for some prop types
- :new should return Arc<Self> directly
- Add doc comments throughout outlook-mapi
- Bump outlook-mapi version
- Use struct syntax with named members in complex SizedXXX macros
- Update the readme
- Take out redundant count params and test macros
- Add SizedXXX macros for all the non-display-table structs
- Misc. cleanup of public API
- Implement a few safe helper structs/macros
- Implement more realistic stateful structs around unsafe fns
